...
#
@hsm
Feature: Off-line signing using HSM managed identities
    Background:
        ...
        And I register and enroll an HSM user HSMOfflineUser in MSP Org1MSP
        And I create a gateway named myhsmgateway without signer for HSM user HSMOfflineUser in MSP Org1MSP
    ...
    Scenario: Evaluate succeeds with off-line signing
        When I prepare to evaluate an echo transaction
        And I set the transaction arguments to ["conga"]
        And I do off-line signing as HSM user HSMOfflineUser in MSP Org1MSP
